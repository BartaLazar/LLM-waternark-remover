...
The same rewrite, from the browser UI:

![synreplace web interface showing a short example rewritten in the Blk+Red view, with its corrections cards below](docs/web-ui-short.png)
...
- **Deterministic.** Same input, same flags, same output, every single time:
...
- **Grammatically correct substitutions.** A replacement is re-conjugated,
  re-pluralized and re-capitalized to match the original word's form, for
  example `expressed` → `evinced` or `Researchers` → `Investigators`. A
  preceding `a`/`an` is fixed too if the replacement changes which one
  belongs (`a single discovery` → `an individual discovery`), decided from
